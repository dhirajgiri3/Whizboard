...
const HelpHero = ({
  ...
  const neutralOrbStyle = {
    background:
      "radial-gradient(circle, rgba(115, 115, 115, 0.15) 0%, rgba(115, 115, 115, 0.03) 50%, transparent 70%)",
    filter: "blur(50px)",
    willChange: "transform",
  };
        ...
        >
          ...
          >
            ...
            <motion.h1
              variants={fadeInUp}
              transition={{ duration: 0.6 }}
              className="max-w-3xl text-3xl sm:text-4xl md:text-5xl lg:text-6xl xl:text-7xl font-bold leading-[1.1] tracking-tight text-white px-2"
            >
              How Can We{" "}
              <motion.span
            ...
            <motion.p
              variants={fadeInUp}
              transition={{ duration: 0.6 }}
              className="text-lg text-white/70 leading-relaxed mb-6 sm:mb-8 lg:mb-10 max-w-xl mx-auto px-4 sm:px-6"
            >
              Find answers to your questions, learn new features, and master
              WhizBoard with our comprehensive documentation and tutorials.
            </motion.p>
          </div>
          ...
          >
            <div className="relative group">
              <Search className="absolute z-1 left-3 sm:left-4 top-1/2 transform -translate-y-1/2 text-white/40 w-4 h-4 sm:w-5 sm:h-5 group-focus-within:text-blue-400 transition-colors duration-200" />
              <input
                type="text"
                placeholder="Search for help articles, tutorials, or features..."
                value={searchQuery}
                onChange={(e) => onSearchChange(e.target.value)}
                className="w-full pl-10 sm:pl-12 pr-10 sm:pr-12 py-3 sm:py-4 bg-white/[0.05] backdrop-blur-sm border border-white/[0.08] rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-blue-500 text-base sm:text-lg placeholder:text-base sm:placeholder:text-lg text-white placeholder-white/40 transition-all duration-200 hover:bg-white/[0.08] hover:border-white/[0.12]"
              />
              {searchQuery && (
                <button
            ...
            <motion.div
              ...
              className="mt-3 sm:mt-4 flex flex-wrap justify-center items-center gap-1.5 sm:gap-2 px-2"
            >
