  ...
  return (
    <div className="relative">
      <button
        ...
        className={`relative p-2 rounded-full ${buttonHover} transition-colors`}
      ...
      >
        <span className="relative inline-flex">
          ...
          {unreadCount > 0 && (
            <span
              className={`absolute top-0 right-0 translate-x-1/4 -translate-y-1/4 w-5 h-5 bg-red-500 text-white text-[10px] font-medium rounded-full flex items-center justify-center ${ringColor} ring-2`}
          ...
          <span
            className={`absolute bottom-0 right-0 translate-x-1/4 translate-y-1/4 w-2.5 h-2.5 ${
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className={`absolute right-0 top-full mt-2 w-80 rounded-xl border z-50 ${dropdownBg} backdrop-blur-md shadow-2xl`}
          >
            <div className={`p-4 border-b ${isLightMode ? 'border-gray-200' : 'border-white/10'}`}>
              <div className="flex items-center justify-between">
                <h3 className={`font-semibold ${headingColor}`}>Notifications</h3>
                <button
            ...
            <div className="max-h-96 overflow-y-auto">
              {notifications.length === 0 ? (
                <div className={`p-4 text-center ${subTextColor}`}>
                  <Bell className={`w-8 h-8 mx-auto mb-2 opacity-50 ${subTextColor}`} />
                    ...
                    >
                      <div className="flex items-start gap-3">
                        ...
                        <div className="flex-1 min-w-0">
                          <p className={`${isLightMode ? 'text-gray-900' : 'text-white'} text-sm leading-relaxed`}>
                        ...
                        {!notification.read && (
                          <div className={`w-2 h-2 rounded-full flex-shrink-0 mt-2 ${isLightMode ? 'bg-blue-500' : 'bg-blue-400'}`} />
                        )}
                      </div>
                    </motion.div>
                  ))}
            ...
            <div className={`p-3 border-t ${isLightMode ? 'border-gray-200' : 'border-white/10'} space-y-2`}>
              ...
              {totalCount > 10 && (
                <button
                  ...
                  className={`w-full text-center ${subTextColor} hover:opacity-80 text-sm transition-colors`}
                >
              ...
              {showAllNotifications && hasMore && (
                <button
                  ...
                  disabled={isLoading}
                  className={`w-full text-center ${subTextColor} hover:opacity-80 text-sm transition-colors disabled:opacity-50`}
                >
